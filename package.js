...
// ══════════════════════════════════════════════════════════════════════
//  ROSH HASHANAH MODE — the Shabbat Dinner package above steps aside for
//  Rosh Hashanah week (Friday, Sept 11) and this fixed-menu package, with
//  its own guest count and fish choice, takes its place on this same
//  page. It automatically reverts to the normal Shabbat Dinner the
//  following week — nothing here needs to be manually switched back.
// ══════════════════════════════════════════════════════════════════════
const RH_PACKAGE = {
...
const RH_INCLUDED_LIST_HTML = `
  <p>— Chaya's Maple Crumble Round Challah</p>
  <p>— Signature Hummus</p>
  <p>— Baba Ganoush</p>
  <p>— Matbucha</p>
  <p>— Seasonal Salad</p>
  <p>— Your choice of salmon (below)</p>
  <p>— Chefaleh Brisket</p>
  <p>— Israeli Pargit</p>
  <p>— Roasted Fingerling Potatoes with Rainbow Onions</p>
  <p>— Green Beans with Silan &amp; Sesame</p>
  <p>— Fresh fruit</p>
`;
...
// isRoshHashanahWeek() comes from cart.js, loaded before this file.
...
function renderRhAddons() {
  const el = document.getElementById('rh-addons-grid');
  if (!el) return;
  const items = RH_ADDON_NAMES
    .map(name => allProducts.find(p => p.name === name && p.visible !== false))
    .filter(Boolean);
...
async function loadRhAddonData() {
...
function initRhMode() {
  document.title = 'Rosh Hashanah Dinner — $125 Per Person | Chefaleh';
  document.getElementById('pkg-name').textContent = RH_PACKAGE.name;
  document.getElementById('pkg-subtitle').innerHTML = '$125 Per Person &nbsp;&middot;&nbsp; 6-Person Minimum';
  document.getElementById('pkg-description').textContent = 'A complete Chefaleh table for the New Year — round challah, dips and salads, your choice of salmon, both brisket and pargit, sides and fresh fruit.';
  document.getElementById('pkg-included-title').textContent = 'Every Rosh Hashanah Dinner Comes With';
  document.getElementById('pkg-included-list').innerHTML = RH_INCLUDED_LIST_HTML;
  const note = document.getElementById('pkg-included-note');
  if (note) note.remove();

  document.getElementById('pkg-builder-section').classList.add('hidden');
  document.getElementById('rh-builder-section').classList.remove('hidden');
  document.getElementById('pkg-extras-section').classList.add('hidden');
  document.getElementById('rh-extras-section').classList.remove('hidden');
